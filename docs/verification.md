...
- the 36-member i386 C/C++/assembly corpus across `-O0/-O1/-O2/-O3/-Os/-Oz`, with 252 independently transformed outputs, deterministic bytes and lineage, LLVM inspection, COFF archive/consumer linking, and ELF i386 relocatable links;
- hard native 32-bit PE execution for original and all seven transformed objects, with i386 machine checks, bounded child runs, exit code 37, and identical fixed result bytes;
...
- nine seed-backed libFuzzer surfaces covering detection, objects, linked images, archives, VM bytecode, TOML configuration, lineage JSON, bounded six-pair machine emission, and transactional object rewriting without executing parsed or emitted native code.
...
`installed_track2_consumer` and `installed_track3_consumer` are first-class CTest gates. Each removes only its build-local prefix, installs the current configuration, verifies the declared archive closure, and compiles against installed headers and libraries in declared link order. Track 2 checks the three providers, typed IR, and capability evidence; Track 3 additionally compiles checked-in i386 fixtures, parses/analyzes both formats, exercises templates, fixups, ABI/unwind plans and all seven transforms, applies each eligible exact-safe pass, verifies explicit refusal or unchanged layout for unowned frame records, then writes/reparses every result.

## Differential verification

Harmless C and assembly object fixtures are compiled, transformed, and linked into independent original and transformed native executables. The transformation fixture guarantees that every eligible balanced machine-code pass changes executed code. A dedicated 32-bit ABI executable validates register cycles, an EAX-sourced stack argument, callee cleanup, and 16-byte call-site alignment; LLVM independently accepts the generated relocatable DWARF CFI. A separate VM-lowering fixture is compiled as both COFF and ELF; its COFF bytes are parsed and lifted while the exact same object is linked into the differential process and executed as the native oracle. Baseline, flattened, outlined, and split programs are lowered, assembled, decoded, and interpreted before comparison. The transformation harness runs an identical six-case input matrix and byte-compares exit status, stdout, stderr, deterministic output files, explicit function results, and global-data side effects. Timing is never captured or used for semantic equivalence.

Generated property tests compare assembled/decoded VM execution against an independent host oracle. Deterministic robustness tests mutate real compiler/linker-produced fixtures and require stable success or structured failure on repeated input. Mutation suites fail unless required corrupt artifact/model cases and all seven i386 rewrite/fixup ownership checks are killed. Seed-backed compiler-native fuzz targets exercise every implemented parser/decoder boundary using bounded in-memory input, including strict configuration, lineage sidecars, and transactional i386 rewrite plans. The code-generation target maps arbitrary bytes to allowlisted tokens for x86, x86-64, or ARM64 and COFF or ELF, applies limits below production defaults, emits twice, and requires identical success or structured failure without executing the result.
