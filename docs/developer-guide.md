...
Size-changing or reordered object work must use `ObjectRewritePlan`, not ad hoc offset edits. Requests must cover a total non-overlapping mapping and identify an owned transform; creation repairs symbols, functions, instructions, blocks, relocation sites/targets/addends, associations, unwind records, and lineage on a private candidate. Validate against the complete immutable source snapshot, then commit the frozen copy. Add targeted tests for PC bias, signed range, implicit addends, association/unwind ownership, overlap, and source-snapshot mismatch.
