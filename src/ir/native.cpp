...
auto variable_valid(const IrFunction& function, IrVariable variable) -> bool {
    return static_cast<std::size_t>(variable.index) < function.variableWidths.size();
}

auto operand_valid(
    const IrFunction& function,
    const IrOperand& operand,
    IrWidth width) -> Result<std::size_t, Diagnostic> {
    if (const auto* variable = std::get_if<IrVariableOperand>(&operand)) {
        ...
        if (function.variableWidths[variable->variable.index] != width) {
            return failure("ir.width_mismatch", "IR operand variable width does not match");
        }
    } else if (std::get<IrImmediateOperand>(operand).width != width) {
        return failure("ir.width_mismatch", "IR immediate width does not match");
    }
...
auto validate_function(const IrFunction& function, const IrLimits& limits)
    ...
    if (function.variableWidths.size() > limits.maxVariables) {
    ...
    for (const auto& argument : function.arguments) {
        ...
        if (function.variableWidths[argument.destination.index] != argument.width) {
            return failure("ir.width_mismatch", "IR argument width does not match its variable");
        }
    }
        ...
        for (const auto& instruction : block.instructions) {
            auto checked = std::visit([&](const auto& item) -> Result<std::size_t, Diagnostic> {
                ...
                if constexpr (std::is_same_v<T, IrMove>
                    ...
                    if (function.variableWidths[item.destination.index] != item.width) {
                        return failure("ir.width_mismatch", "IR destination width does not match");
                    }
                    return operand_valid(function, item.source, item.width);
                } else if constexpr (std::is_same_v<T, IrUnaryOperation>) {
                    if (!variable_valid(function, item.destination)) {
                        return failure("ir.variable_out_of_range", "IR destination variable is out of range");
                    }
                    ...
                    if (function.variableWidths[item.left.index] != item.width) {
                        return failure("ir.width_mismatch", "IR comparison width does not match");
                    }
                    return operand_valid(function, item.right, item.width);
                } else if constexpr (std::is_same_v<T, IrReturn>) {
                    if (!variable_valid(function, item.value)) {
                        return failure("ir.variable_out_of_range", "IR return variable is out of range");
                    }
                    if (item.width != function.returnWidth
                        || function.variableWidths[item.value.index] != item.width) {
                        return failure("ir.width_mismatch", "IR return width does not match");
                    }
                } else if constexpr (std::is_same_v<T, IrInternalCall>) {
                    ...
                    if (function.variableWidths[item.destination.index] != item.resultWidth) {
                        return failure(
                            "ir.width_mismatch",
                            "IR internal-call result width does not match its destination");
                    }
    ...
    const auto variableCount = function.variableWidths.size();
        ...
        for (const auto& block : function.blocks) {
            for (const auto& instruction : block.instructions) {
                ...
                if (call->arguments.size() != target.arguments.size()
                    || call->resultWidth != target.returnWidth) {
                    return failure(
                        "ir.internal_call_signature_mismatch",
                        "IR internal call does not match the target signature");
                }
                std::vector<IrWidth> targetWidths(target.arguments.size());
                for (const auto& argument : target.arguments) {
                    targetWidths[argument.argumentIndex] = argument.width;
                }
                for (std::size_t index = 0; index < call->arguments.size(); ++index) {
                    IrWidth actualWidth = IrWidth::U32;
                    if (const auto* variable = std::get_if<IrVariableOperand>(
                            &call->arguments[index])) {
                        actualWidth = function.variableWidths[variable->variable.index];
                    } else {
                        actualWidth = std::get<IrImmediateOperand>(
                            call->arguments[index]).width;
                    }
                    if (actualWidth != targetWidths[index]) {
                        return failure(
                            "ir.internal_call_signature_mismatch",
                            "IR internal-call argument width does not match the target");
                    }
                }
